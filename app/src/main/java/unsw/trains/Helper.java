...
public class Helper {
    ...
    public static void simulateLoadEmbark(Station st, Train t) {
        ...
        for (Load load : loads) {
            if (doesTrainReachDestination(t, load) &&
                ...
                if (load instanceof PassengerLoad && t.canPassengersBeOnThisTrain())
                    embark = true;
                else if (load instanceof CargoLoad && t.canCargoBeOnThisTrain())
                    embark = true;
                else if (load instanceof PerishableCargoLoad && t.canCargoBeOnThisTrain()) {
                    PerishableCargoLoad oth = (PerishableCargoLoad) load;
                    if (oth.shouldPerishableBeEmbarked(t, st))
                        embark = true;
                }
    ...
    public static Track isThereATrack(HashMap<String, Track> tracks, String st1, String st2) {
        for (Track track : tracks.values()) {
            if ((track.getFromStationId().equals(st1) && track.getToStationId().equals(st2))
                    || (track.getFromStationId().equals(st2) && track.getToStationId().equals(st1)))
                return track;
        }
        return null;
    }
    ...
    public static List<Train> trainsOnStation(List<Train> trains, Station st) {
        ...
        for (Train train : trains) {
            if (train.getTrainPosition().equals(st.getStationCoordinates()))
                res.add(train);
        }
    ...
    public static void fixDurabilityOfTrack(RepairTrain t, UnbrokenTrack track) {
        ...
        if (track.getDurability() == 10)
            track.setTrackType(TrackType.UNBROKEN);
    }
    ...
    public static List<String> isRouteValid(HashMap<String, Track> tracks, List<String> route)
        ...
        if (t == null)
            ...
            throw new InvalidRouteException("Route cannot be circular!!");
    }
    ...
    public static Station createStationHelper(String stationId, String type, double x, double y) {
        Station st;
        if (Objects.equals("PassengerStation", type))
            st = new PassengerStation(stationId, type, x, y);
        else if (Objects.equals("CargoStation", type))
            st = new CargoStation(stationId, type, x, y);
        else if (Objects.equals("DepotStation", type))
            st = new DepotStation(stationId, type, x, y);
        else if (Objects.equals("CentralStation", type))
            st = new DepotStation(stationId, type, x, y);
        else
            st = null;
        return st;
    ...
    public static Train createTrainHelper(String trainId, String type, String stationId, List<String> route,
        ...
        Train t;
        if (Objects.equals("PassengerTrain", type))
            t = new PassengerTrain(trainId, type, stationId, route, stations.get(stationId).getStationCoordinates(),
                    tracks);
        else if (Objects.equals("BulletTrain", type))
            t = new BulletTrain(trainId, type, stationId, route, stations.get(stationId).getStationCoordinates(),
                    tracks);
        else if (Objects.equals("CargoTrain", type))
            t = new CargoTrain(trainId, type, stationId, route, stations.get(stationId).getStationCoordinates(),
                    tracks);
        else if (Objects.equals("RepairTrain", type))
            t = new RepairTrain(trainId, type, stationId, route, stations.get(stationId).getStationCoordinates(),
                    tracks);
        else
            t = null;
        return t;
    ...
    public static void helperSimulate(HashMap<String, Train> trains, HashMap<String, Station> stations,
        ...
        for (String id : tr) {
            ...
            if (stationFinal.checkIfStationIsFull(new ArrayList<>(trains.values())))
                continue;
            ...
            InterfaceTrainAndCargo canCarryCargo = (t instanceof InterfaceTrainAndCargo) ? (InterfaceTrainAndCargo) t
                    : null;

            if (canCarryCargo != null)
                canCarryCargo.decreaseTrainSpeed();
            ...
            if (canCarryCargo != null)
                canCarryCargo.removeExpiredCargo();
